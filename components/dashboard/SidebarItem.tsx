...
export const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  const content = (
    <>
      <Icon
        size={20}
        className={`h-5 w-5 shrink-0 transition-transform duration-200 group-hover:scale-110 ${active ? "text-white" : "text-zinc-500 group-hover:text-zinc-900 dark:text-zinc-400 dark:group-hover:text-white"
          }`}
      />
      ...
      {!isCollapsed && badge !== undefined && (
        <span
          className={`ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[11px] font-bold ${active ? "bg-white/20 text-white" : badgeColor
            }`}
        >
          {badge}
        </span>
  ...
  const className = `group relative flex w-full items-center gap-3.5 rounded-xl px-3.5 py-3 text-sm font-semibold transition-all duration-200 ${isCollapsed ? "justify-center px-0 py-3" : ""
    } ${active
