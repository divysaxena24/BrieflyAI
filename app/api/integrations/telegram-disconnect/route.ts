...
export async function GET(request: Request) {
  ...
  // Invalidate the stored bot token (nulls tokens + marks not-connected)
  await telegramTokenManager.invalidate(integration.id);
  ...
  logger.info("Telegram disconnected", { userId, integrationId: integration.id, platform });

  return NextResponse.json({ message: "Disconnected Telegram" });
}
